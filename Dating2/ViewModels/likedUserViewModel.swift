...
class likedUserViewModel: ObservableObject {
            ...
            for liked in snapshot.children {
                ...
                ref.child("likedUsers").child(id).observe(.value, with: { snapshot in
                    ...
                    if (snapshot.value is NSNull) {
                        ...
                        if uid == likedUser.uid2 {
                            ref.child("users").queryOrdered(byChild: "uid").queryEqual(toValue: likedUser.likedUser).observeSingleEvent(of: .childAdded, with: { snapshot in
